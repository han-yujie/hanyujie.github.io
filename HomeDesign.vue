...
.badge { background: #8B5CF6; color: white; padding: 4px 12px; border-radius: 12px; font-size: 12px; font-weight: bold; }
...
.btn { padding: 10px 30px; border-radius: 50px; font-weight: 600; text-decoration: none; display: inline-block; transition: 0.3s; }
.btn.primary { background: #8B5CF6; color: white !important; }
...
.resume-content { display: flex; align-items: center; justify-content: space-between; width: 100%; max-width: 1000px; gap: 40px; }
.resume-intro { flex: 1.2; }
.resume-intro h3 { font-size: 2.2rem; font-weight: 800; color: var(--vp-c-text-1); margin: 0 0 10px; }
.highlight-text { color: #8B5CF6; }
.resume-subtitle { margin: 0 0 25px; color: var(--vp-c-text-2); font-size: 1.1rem; }
.btn-resume-cta { 
  background: #8B5CF6; color: white !important; padding: 14px 32px; border-radius: 50px; 
  font-weight: 700; text-decoration: none; box-shadow: 0 6px 16px rgba(139, 92, 246, 0.25);
}
.resume-stats-container { flex: 1; display: flex; flex-direction: column; align-items: center; }
.resume-stats { display: flex; gap: 40px; margin-bottom: 25px; justify-content: center; width: 100%; }
.stat-item { display: flex; flex-direction: column; align-items: center; }
.stat-val { font-size: 2.8rem; font-weight: 800; color: #8B5CF6; line-height: 1; }
.stat-lbl { font-size: 0.85rem; color: var(--vp-c-text-2); margin-top: 10px; font-weight: 600; }
.resume-skills-tags { display: flex; gap: 12px; flex-wrap: wrap; justify-content: center; }
.resume-skills-tags span { font-size: 0.75rem; padding: 4px 12px; background: var(--vp-c-bg-mute); border-radius: 20px; border: 1px solid var(--vp-c-divider); }

/* === 5. 列表与卡片布局 === */
.uniform-grid, .aigc-grid, .tools-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 24px; }
.work-card, .aigc-card { position: relative; aspect-ratio: 4 / 3; }
.img-wrap { width: 100%; height: 100%; position: relative; overflow: hidden; } 
.img-wrap img { width: 100%; height: 100%; object-fit: cover; }
.info, .aigc-info { position: absolute; bottom: 0; padding: 20px; width: 100%; background: linear-gradient(transparent, rgba(0,0,0,0.8)); }
.white-text { color: white !important; margin: 5px 0 0; }
.blog-card { display: flex; height: 280px; margin-bottom: 24px; }
.blog-left { width: 40%; }
.blog-left img { width: 100%; height: 100%; object-fit: cover; }
.blog-right { width: 60%; padding: 30px; display: flex; flex-direction: column; justify-content: space-between; }
.tool-card { padding: 30px; text-align: center; }
.btn-download { background: #8B5CF6; color: white !important; padding: 8px 20px; border-radius: 8px; text-decoration: none; margin-top: 15px; display: block; }

/* === 6. 联系方式与底部 === */
.contact-card { display: flex; padding: 50px; justify-content: space-between; align-items: center; }
.contact-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 15px; }
.contact-item { display: flex; align-items: center; gap: 10px; padding: 12px 20px; border: 1px solid var(--vp-c-divider); border-radius: 12px; text-decoration: none !important; }
.c-icon { width: 24px; height: 24px; }
.back-to-top { position: fixed; bottom: 30px; right: 30px; width: 50px; height: 50px; border-radius: 50%; background: #8B5CF6; color: white; border: none; cursor: pointer; z-index: 100; }
.modal-overlay { position: fixed; inset: 0; background: rgba(0,0,0,0.7); display: flex; align-items: center; justify-content: center; z-index: 1000; }
.modal-content { background: var(--vp-c-bg); padding: 30px; border-radius: 20px; text-align: center; }
.modal-img { max-width: 250px; border-radius: 10px; }

/* =================================================================
   📱 移动端适配 (Mobile Force Fix) - 彻底解决文案溢出
   ================================================================= */
@media (max-width: 768px) {
  /* 基础容器 */
  .home-container { padding: 0 15px 60px !important; }
  .section-container { margin-top: 50px !important; }

  /* Hero 区域：文字缩小且不溢出 */
  .hero-section { flex-direction: column !important; text-align: center !important; min-height: auto !important; padding-top: 10px !important; }
  .hero-title { 
    font-size: 1.8rem !important; /* 强制缩小标题 */
    line-height: 1.3 !important;
    word-break: break-word !important;
  }
  .hero-desc { font-size: 0.95rem !important; line-height: 1.5 !important; }
  .hero-right { width: 100% !important; height: 260px !important; margin-top: 20px !important; }
  .hero-img { width: 90% !important; }

  /* 简历板块：彻底优化 */
  .resume-bar { padding: 30px 15px !important; margin-bottom: 40px !important; }
  .resume-content { flex-direction: column !important; gap: 25px !important; text-align: center !important; }
  .resume-intro flex { width: 100% !important; }
  .resume-intro h3 { 
    font-size: 1.5rem !important; /* 缩小名字标题 */
    line-height: 1.2 !important;
  }
  .resume-subtitle { font-size: 0.85rem !important; margin-bottom: 20px !important; }
  .btn-resume-cta { width: 100% !important; padding: 12px 20px !important; font-size: 0.95rem !important; }

  /* 简历数字板块 */
  .resume-stats { gap: 15px !important; }
  .stat-val { font-size: 1.8rem !important; } /* 缩小数字 */
  .stat-lbl { font-size: 0.75rem !important; white-space: nowrap !important; }
  .resume-skills-tags { gap: 8px !important; }
  .resume-skills-tags span { font-size: 0.7rem !important; padding: 3px 10px !important; }

  /* 通用标题：强制不溢出 */
  .common-header { margin-bottom: 30px !important; }
  .common-header h3 { 
    font-size: 1.4rem !important; /* 缩小板块汉字标题 */
    padding: 0 10px !important;
  }
  .common-header p { font-size: 0.85rem !important; }

  /* 网格与卡片 */
  .uniform-grid, .aigc-grid, .tools-grid { grid-template-columns: 1fr !important; gap: 16px !important; }
  .white-text { font-size: 1.1rem !important; }
  ...
  .blog-main h4 { font-size: 1.1rem !important; line-height: 1.4 !important; }
  .blog-main p { font-size: 0.85rem !important; -webkit-line-clamp: 2 !important; }

  /* 联系方式 */
  .contact-card { flex-direction: column !important; padding: 30px 15px !important; text-align: center !important; }
  .contact-left h3 { font-size: 1.4rem !important; }
  .contact-right { width: 100% !important; margin-top: 20px !important; }
  .contact-grid { grid-template-columns: 1fr !important; } /* 强制单列 */
  
  /* 按钮与弹窗 */
  .back-to-top { bottom: 15px !important; right: 15px !important; width: 42px !important; height: 42px !important; }
  .modal-img { max-width: 200px !important; }
}
</style>